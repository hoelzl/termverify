...
Slice 2 (issue #143, PR #144) is implemented and under adversarial review;
if accepted it completes the workstream:
`tests/test_conpty_integration.py` carries the durable
...
stays fail-closed on the real adapter with OS-observed teardown. Once both
authorized slices of the key-to-terminal byte mapping design are merged,
further key-input work (an extended encoding version, POSIX, key-support
negotiation) requires a new owner decision.
