...
| `peer-malformed` | A child message is not a valid v1 message: malformed JSON, wrong protocol tag, unknown kind, missing or misspelled required member, non-`x-` extension violation, a payload-value rule the codec owns (such as exited-process evidence inside `session.ready`), or a resource-limit breach. |
| `peer-lifecycle` | A structurally valid message arrives out of lifecycle position: traffic before `session.hello` could only be a race on reused pipes, a second readiness, an observation with no epoch open, an input's closing message never arriving before the next input, an epoch or drain observation carrying exited-process evidence, or traffic after a terminal message. |
