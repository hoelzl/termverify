- **Tests/CI minors from the 2026-07-24 adversarial review are swept**
  ...
  still suspended, so exit-code evidence can never race PID reaping; the
  remaining arrangement sleeps were audited and are each documented as
  arrangement-not-evidence. Also: a direct invalid-UTF-8 fixture pins
  ...
  child grew public `fail_next_reads`/`fail_writes` arrangement methods,
