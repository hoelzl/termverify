...
merge conflicts in multi-agent work; new files in this directory never
conflict.
...
```
changelog.d/160-release-pipeline.added.md
changelog.d/157-conpty-teardown.fixed.md
```
...
Do **not** edit the `[Unreleased]` section of `CHANGELOG.md` in a PR. Release
notes describing breaking protocol or API changes still carry their migration
note inside the fragment, under a `changed` or `removed` type.

## Releasing

At release time (step 1 of `docs/developer-guide/release.md`):

```
uv --no-config run python scripts/collect_changelog.py X.Y.Z
```
