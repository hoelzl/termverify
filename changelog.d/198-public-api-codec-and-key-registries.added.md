- **The authoritative transcript codec and the key registries' entry points
  are now on the public `termverify` surface.** `parse_transcript`,
  ...
  names are public, their defining modules are not, and the rest of those
  modules (including the `all_key_chords` enumeration) stays private.
  `encode_key_chord`'s annotation is narrowed from `Sequence[str]` to
  `list[str] | tuple[str, ...]` to match what it has always accepted; no
