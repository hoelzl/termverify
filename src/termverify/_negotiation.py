...
Extracted unchanged from the direct adapter so every adapter negotiates the
same way: constraints run in ``CONSTRAINT_NAMES`` order, each step must
return the matching receipt bound to the run and the requested value, a
``ConstraintUnsupported`` report ends the start as ``StartUnsupported``, and
anything else — a raising port, a mismatched receipt, a wrong-constraint
report — fails closed as ``StartFailed``.
"""
...
def negotiate(
    ...
    for constraint, operation, receipt_type, expected_value in zip(
        CONSTRAINT_NAMES, operations, _RECEIPT_TYPES, expected_values, strict=True
    ):
