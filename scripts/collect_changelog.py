...
Usage:
    python scripts/collect_changelog.py 0.2.0
    python scripts/collect_changelog.py 0.2.0 --date 2026-07-20
    python scripts/collect_changelog.py 0.2.0 --dry-run
...
See ``changelog.d/README.md`` for the fragment conventions. Adapted from
the equivalent tooling in the sibling clm project.
"""
    ...
    def flush() -> None:
        ...
        if current is None:
            raise ChangelogError(
                "found content in [Unreleased] outside any '### Section' "
                f"heading:\n{block[:200]}"
            )
...
def build_release_section(
    ...
    for section in SECTIONS:
        blocks = ([leftover[section]] if section in leftover else []) + fragments.get(
            section, []
        )
        if blocks:
            parts.append(f"\n### {section}\n\n")
...
def collect(version: str, release_date: str, dry_run: bool) -> int:
    ...
    collected = [
        path
        for path in sorted(FRAGMENTS_DIR.iterdir())
        if path.name not in NON_FRAGMENTS
    ]
