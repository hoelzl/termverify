...
All notable changes to the termverify package are documented in this file.
...
def run(*argv: str) -> int:
    ...
    try:
        return int(collect_changelog.main())
    finally:
        sys.argv = old_argv


class TestCollect:
    def test_fragments_become_release_section(self, fake_repo: Path) -> None:
        frags = fake_repo / "changelog.d"
        (frags / "101-feature.added.md").write_text(
            "- **Feature.** Does X.\n", encoding="utf-8"
        )
        (frags / "102-bug.fixed.md").write_text(
            "- **Bug.** Fixed Y.\n", encoding="utf-8"
        )

        assert run("0.2.0", "--date", "2026-07-20") == 0

        text = (fake_repo / "CHANGELOG.md").read_text(encoding="utf-8")
        added = text.index("## [0.2.0] - 2026-07-20")
        ...
        assert text.index("## [0.1.0] - 2026-07-19") > text.index("- **Bug.** Fixed Y.")
        # Unreleased heading + note stay on top for the next cycle.
        assert text.index("## [Unreleased]") < added
        assert NOTE in text

    def test_collected_fragments_are_deleted_readme_kept(self, fake_repo: Path) -> None:
        frags = fake_repo / "changelog.d"
    ...
    def test_sections_in_canonical_order_fragments_by_filename(
        self, fake_repo: Path
    ) -> None:
        frags = fake_repo / "changelog.d"
    ...
    def test_leftover_unreleased_entries_fold_in_before_fragments(
        self, fake_repo: Path
    ) -> None:
        ...
        unreleased = result[
            result.index("## [Unreleased]") : result.index("## [0.2.0]")
        ]
        assert "hand-written" not in unreleased

    def test_default_date_is_today(self, fake_repo: Path) -> None:
        ...
        assert run("0.2.0") == 0
        text = (fake_repo / "CHANGELOG.md").read_text(encoding="utf-8")
        assert f"## [0.2.0] - {date.today().isoformat()}" in text
    ...
    def test_dry_run_modifies_nothing(
        ...
        assert run("0.2.0", "--date", "2026-07-20", "--dry-run") == 0
        assert (fake_repo / "CHANGELOG.md").read_text(encoding="utf-8") == before
        assert (frags / "101-feature.added.md").exists()
        assert "## [0.2.0] - 2026-07-20" in capsys.readouterr().out


class TestErrors:
    def expect_unchanged_failure(self, fake_repo: Path, *argv: str) -> str:
        ...
        assert run(*argv) == 1
        assert (fake_repo / "CHANGELOG.md").read_text(
            encoding="utf-8"
        ) == before_changelog
        assert (
            sorted(p.name for p in (fake_repo / "changelog.d").iterdir())
            == before_files
        )
        return before_changelog

    def test_unknown_fragment_type(
        self, fake_repo: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        (fake_repo / "changelog.d" / "1-x.adged.md").write_text(
            "- e\n", encoding="utf-8"
        )
        self.expect_unchanged_failure(fake_repo, "0.2.0")
        assert "unknown fragment type" in capsys.readouterr().err

    def test_unrecognized_file_name(
        self, fake_repo: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
    ...
    def test_nothing_to_collect(
        self, fake_repo: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        self.expect_unchanged_failure(fake_repo, "0.2.0")
        assert "nothing to collect" in capsys.readouterr().err

    def test_version_already_released(
        self, fake_repo: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        (fake_repo / "changelog.d" / "1-x.added.md").write_text(
            "- e\n", encoding="utf-8"
        )
        self.expect_unchanged_failure(fake_repo, "0.1.0")
    ...
    def test_invalid_version(
        ...
        self.expect_unchanged_failure(fake_repo, "0.2")
        assert "not an X.Y.Z version" in capsys.readouterr().err

    def test_unknown_unreleased_section_heading(
        self, fake_repo: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        changelog = fake_repo / "CHANGELOG.md"
        text = changelog.read_text(encoding="utf-8")
    ...
    def test_unreleased_content_outside_section(
        self, fake_repo: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        changelog = fake_repo / "CHANGELOG.md"
        text = changelog.read_text(encoding="utf-8")
