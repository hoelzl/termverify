...
| Transcript-protocol conformance and compatibility (`termverify.transcript/v1`) | Runtime validation in `src/termverify/transcript.py` is authoritative for protocol acceptance; the packaged Draft 2020-12 schema (`termverify.transcript_schema_v1_bytes()`) is a non-exhaustive structural aid — schema acceptance is not conformance. Policy and rationale: `docs/knowledge/protocol.md`. Current freeze and compatibility-stage status: `docs/agent/design/prototyping-stage-protocol-governance.md` |
| Control-protocol acceptance (`termverify.control/v1`) | The strict codec in `src/termverify/control.py` is authoritative for wire acceptance; `docs/knowledge/control-protocol.md` is the normative specification. On disagreement the codec wins, per the executable-over-prose rule in *Start Here*, and the mismatch is a defect: repaired doc-side by default, code-side through a test-first slice when the codec is the wrong one. Stage scope and revision of this polarity: `docs/agent/design/prototyping-stage-protocol-governance.md` |
